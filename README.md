...
- **高效管理**：自动化成绩录入、统计和分析，减轻教师和管理人员的工作负担
- **便捷服务**：学生可以随时查询成绩，了解学习情况
- **数据安全**：集中存储成绩数据，避免数据丢失和篡改
- **决策支持**：提供成绩分析和统计功能，为教学管理决策提供数据支撑
...
| 接口 | 路径 | 方法 | 说明 |
|------|------|------|------|
| 管理员登录 | `/admin/login` | POST | 管理员账号登录 |
| 学生登录 | `/student/login` | POST | 学生账号登录 |
| 教师登录 | `/teacher/login` | POST | 教师账号登录 |
...
| Java 类 | 驼峰命名（首字母大写） | `ScoreDetailServiceImpl` |
| Java 变量 | 驼峰命名（首字母小写） | `isMakeup`, `originalScore` |
| 前端变量 | 驼峰命名（首字母小写） | `selectedClassId`, `studentList` |
